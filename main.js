...
(function($) {
    ...
    $('#contact-form').on('submit', function(event) {
      ...
      if (isValidForm()) {
		...
		$.ajax({
			...
			data:  {tipoEmail: "contacto", nombre: nombre, email: email, telefono: telefono, mensaje: mensaje },
			success: function (res) {
				//console.log("resultado: " + html);					
				if(res =='OK')
				{
					var message = $('#contact-form').data('success-text') || 'Tu mensaje ha sido enviado. ¡Nos pondremos en contacto con usted en breve!';
				...
				{
					var message = $('#contact-form').data('error-text') || 'Hubo un error, intente nuevamente en unos minutos.' + res;
			...
			error: function (request, status, error) {
				...
				var message = $('#contact-form').data('error-text') || 'Hubo un error, intente nuevamente en unos minutos.';
				var errorTemplate = '<div role="alert" class="alert alert-danger alert-colored">' + message + '</div>';
				$('#contact-form .alert').fadeOut(300);
				$(errorTemplate).insertBefore($('#contact-form .btn'));
			}
  ...
  function initVisitaForm() {
   
   var requiredVisitaInputs = $('#formVisita').find('input[data-required], textarea[data-required]').toArray();

    var isValidVisitaForm = function() {
      var toReturn = true;

      requiredVisitaInputs.forEach(function(element, index) {
        if (!$(element).val()) {
   ...
   $('#formVisita').on('submit', function(event) {
      event.preventDefault();
      ...
      if (isValidVisitaForm()) {
        var nombre = $("#fvisitaNombre").val();
		var email = $("#fvisitaEmail").val();	
		var telefono = $("#fvisitaTelefono").val();	
		var mensaje = $("#fvisitaMensaje").val();
		var fecha = $("#fvisitaFecha").val();
		var hora = $("#fvisitaHora").val();
		var tipo = $("#fvisitaTipo").val();		
		console.log("Enviando reservar.....#2");
		$.ajax({
			type: "POST",
			url: "/mail.php",
			ContentType: 'application/json',
			data:  {tipoEmail: "reserva", nombre: nombre, email: email, telefono: telefono, mensaje: mensaje, fecha: fecha, hora: hora, tipo: tipo },
			success: function (res) {
				//console.log("resultado: " + html);					
				if(res =='OK')
				{
					var message = $('#formVisita').data('success-text') || 'Tu mensaje ha sido enviado. ¡Nos pondremos en contacto con usted en breve!';
					var succesTemplate = '<div role="alert" class="alert alert-success alert-colored">' + message + '</div>';
					$('#formVisita input, #formVisita textarea, #formVisita button').attr('disabled', 'disabled');
					$('#formVisita .alert').fadeOut(300);
					$(succesTemplate).insertBefore($('#formVisita .btn'));
				}
				else
				{
					var message = $('#formVisita').data('error-text') || 'Hubo un error, intente nuevamente en unos minutos.' + res;
					var errorTemplate = '<div role="alert" class="alert alert-danger alert-colored">' + message + '</div>';
					$('#formVisita .alert').fadeOut(300);
					$(errorTemplate).insertBefore($('#formVisita .btn'));
				}
			},
			error: function (request, status, error) {
				console.log("Error: " + request.status + " - " + error);	
				var message = $('#formVisita').data('error-text') || 'Hubo un error, intente nuevamente en unos minutos.';
				var errorTemplate = '<div role="alert" class="alert alert-danger alert-colored">' + message + '</div>';
				$('#formVisita .alert').fadeOut(300);
				$(errorTemplate).insertBefore($('#formVisita .btn'));
			}
